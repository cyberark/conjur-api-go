...
if [ -z "$INFRAPOOL_TEST_CLOUD" ]; then
  ...
  -e USER_ASSIGNED_IDENTITY_CLIENT_ID \
  -e TEST_GCP \
  -e GCP_PROJECT_ID \
  -e GCP_ID_TOKEN \
  -e TEST_CERT \
  -e TEST_CERT_SERVICE_ID \
  -e TEST_CERT_SPIFFE_SERVICE_ID \
  ...
  docker run \
    ...
    "test-$GO_VERSION" bash -c 'set -xo pipefail;
            ...
            cat "$output_dir/junit.output" | go-junit-report > "$output_dir/junit.xml";
            gocov convert "$output_dir/c.out" | gocov-xml > "$output_dir/coverage.xml";
            gocovmerge "./output/1.25/c.out" "$output_dir/c.out" > "$output_dir/merged-coverage.out";
